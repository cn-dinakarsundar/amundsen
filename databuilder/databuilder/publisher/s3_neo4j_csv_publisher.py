...
class S3Neo4jCsvPublisher(Publisher):
    """
    ...
    def __init__(self) -> None:
        super(S3Neo4jCsvPublisher, self).__init__()
    ...
    def init(self, conf: ConfigTree) -> None:
        ...
        self.s3_bucket = conf.get_string(NODE_S3_BUCKET)
        self._node_files = self._list_files(conf, NODE_S3_BUCKET, NODE_S3_PREFIX)
        self._node_files_iter = iter(self._node_files)

        self._relation_files = self._list_files(conf,NODE_S3_BUCKET,RELATION_S3_PREFIX)
    ...
    def _list_files(self, conf: ConfigTree, s3_bucket_key: str, s3_bucket_prefix: str) -> List[str]:
        """
        ...
        """
        if s3_bucket_key not in conf:
            return []

        s3_bucket = conf.get_string(s3_bucket_key)
        s3_prefix = conf.get_string(s3_bucket_prefix)
        s3 = boto3.resource('s3')
        my_bucket = s3.Bucket(s3_bucket)
        s3_path=[]

        for s3_object in my_bucket.objects.filter(Prefix=s3_prefix):
            if s3_object.key != s3_prefix:
                s3_path.append(s3_object.key)
    ...
    def publish_impl(self) -> None:  # noqa: C901
        ...
        for node_file in self._node_files:
            self._create_indices(s3_bucket=self.s3_bucket  ,node_file=node_file)
        ...
        try:
            ...
            while True:
                try:
                    ...
                    tx = self._publish_node(s3_bucket=self.s3_bucket, node_file=node_file , tx=tx)
            ...
            while True:
                try:
                    ...
                    tx = self._publish_relation(s3_bucket=self.s3_bucket, relation_file=relation_file , tx=tx)
                except StopIteration:
                    break
    ...
    def _create_indices(self,s3_bucket: str , node_file: str) -> None:
        """
        Go over the node file and try creating unique index
        :param node_file:
        :return:
        """
        print('Creating indices. (Existing indices will be ignored)')

        s3 = boto3.client('s3')
        #obj = s3.get_object(Bucket=s3_bucket, Key=node_file)
        for node_record in pandas.read_csv("s3://"+s3_bucket+"/"+node_file).to_dict(orient="records"):
    ...
    def _publish_node(self,s3_bucket: str ,node_file: str, tx: Transaction) -> Transaction:
        """
    ...
    def _publish_relation(self, s3_bucket: str, relation_file: str, tx: Transaction) -> Transaction:
        ...
        if self._relation_preprocessor.is_perform_preprocess():
            ...
            for rel_record in pandas.read_csv("s3://" + s3_bucket + "/" + relation_file).to_dict(orient="records"):
